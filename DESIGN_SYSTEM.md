...
| `.confianca` + `.confianca-foto` + `.confianca-selo` + `.checks` | Bloco `--azul` com presença (padding 104px no desktop), grade .9/1.1 com 72px de vão: foto 5:4 de pessoa em atendimento (Unsplash, crédito em CREDITS) com selo amarelo `--amarelo` sobreposto à borda direita ("Fale com um / Especialista Cruzeirista", as duas linhas em 18px alinhadas à esquerda, raio 14, sombra), título `clamp(32px, 6.8vw, 50px)`, parágrafo 19px a 86%, dois benefícios com marcadores redondos amarelos e check escuro. Copy fechada por Lippe em 18/09; sem "atendimento humano", suporte na viagem ou guia digital | bloco azul do rascunho |
